...
- Use native GitHub issue dependencies for blocking where supported.
- The frontier is the first open, unassigned, unblocked child in map order.
- Claim with `gh issue edit <n> --add-assignee @me`.
- Resolve with a comment, close, then append a context pointer to the map's Decisions-so-far.
